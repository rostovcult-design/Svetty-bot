...
await update.message.reply_text(" Напиши подпись для поста (или `-` чтобы без подписи):
...
if not files:
await update.message.reply_text(" return ConversationHandler.END
Не удалось скачать. Пост должен быть публичны
await update.message.reply_text(f" Скачано {len(files)} файл(ов). Постю в канал..."
...
if is_video(f):
await bot.send_video(chat_id=CHANNEL, video=fh, caption=caption, supports_str
else:
...
media.append(InputMediaVideo(fh, caption=cap) if is_video(f) else InputMediaPhoto
try:
