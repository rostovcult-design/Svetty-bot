...
ПЕРВАЯ СТРОКА ПОСТА — выбери ОДНУ фразу которая лучше всего подходит к контенту:

— ты видела это? (для новости, фото, инфоповода)
— я не понимаю, это гениально или провал (для спорного)
— сохрани это, потом обсудим почему это сейчас везде (для тренда)
— это сейчас носят все, но давай честно… (для модного тренда)
— сохрани это (для красивого, эстетичного)
— обсудим? (для неоднозначного)
— это кто вообще одобрил (для WTF)
— тихо происходит (для скрытого тренда)

Формат поста (строго соблюдай):

[одна фраза из списка выше — обычный текст]
...
- ОБЯЗАТЕЛЬНО меняй первую фразу в зависимости от контента"""
